...
const authKey = 'pediflow_token'
let token = localStorage.getItem(authKey)
let currentUser = null
let state = null
let view = 'overview'; let edit = null; let separationDate = today; let ordersDate = today; let ordersFilter = 'created'; let reportDate = today; let reportMode = 'day'; let reportWeekStart = null; let groupByClient = false
...
const visibleNav = () => currentUser?.role === 'owner' ? [...nav, ['administration', '♙', 'Administracao']] : nav
...
const deleteLabels = { client: 'cliente', product: 'produto', order: 'pedido', list: 'lista de compras', user: 'usuario' }
...
const confirmDelete = async () => { if (!pendingDelete) return; const { type, id: itemId } = pendingDelete; if (type === 'user') { const response = await fetch(apiUrl(`/api/admin/users/${itemId}`), { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }); const result = await response.json().catch(() => ({})); if (!response.ok) return flash(result.error || 'Nao foi possivel excluir o usuario.'); closeDeleteModal(); flash('Usuario excluido'); return loadAdminUsers() } if (type === 'client') state.clients = state.clients.filter((item) => item.id !== itemId); if (type === 'product') state.products = state.products.filter((item) => item.id !== itemId); if (type === 'order') state.orders = state.orders.filter((item) => item.id !== itemId); if (type === 'list') state.shopping = state.shopping.filter((item) => item.id !== itemId); save(); closeDeleteModal(); ({ client: clients, product: products, order: orders, list: shopping })[type]() }
...
function orderFileName(order) { const name = client(order.clientId)?.name || 'Cliente_removido'; return `Pedido_${name.replace(/[^a-z0-9]+/gi, '_')}_${date(order.delivery).replaceAll('/', '-')}.pdf` }
...
function rowsGroupedByClient(orders) {
  const groups = orders.reduce((map, order) => {
    const key = order.clientId || 'removed'
    const group = map.get(key) || { clientId: order.clientId, orders: [] }
  ...
  const groupRows = [...groups.values()].map((group) => {
    const c = client(group.clientId)
    const count = group.orders.length
    const groupTotal = ordersTotal(group.orders)
    const groupId = group.clientId || 'removed'

    return `<div class="client-group" data-group-client="${groupId}"><div class="client-group-header" data-toggle-group="${groupId}"><button type="button" class="client-toggle" aria-label="Expandir pedidos do cliente" title="Expandir"><span class="toggle-icon">▼</span></button><span class="avatar soft">${c?.name.split(' ').map((n) => n[0]).slice(0, 2).join('')}</span><strong>${c?.name || 'Removido'}</strong><span class="client-meta"><span class="client-count">${count} ${count === 1 ? 'Pedido' : 'Pedidos'}</span><span class="client-total">${money(groupTotal)}</span></span></div><div class="client-group-orders">${group.orders.map((o) => { const [label, cls] = status(o.status); return `<div class="table-row"><span class="items-summary">${o.items.map((i) => `${i.quantity}x ${product(i.productId)?.name}`).join(', ')}</span><strong>${money(total(o))}</strong><span>${date(o.delivery)}</span><span class="status ${cls}"><i></i>${label}</span><div class="row-actions"><button data-edit-order="${o.id}">✎</button><button data-delete-order="${o.id}">×</button></div></div>` }).join('')}</div></div>`
  }).join('')

  return `${rowsWithTotal(orders).replace('<div class="table-head">', '<div class="table-head"><span></span>')}${groupRows}`
}
...
function orders() {
  ...
  document.querySelector('#toggle-group').onclick = () => { groupByClient = !groupByClient; orders() }
  document.querySelectorAll('.client-toggle').forEach((button) => button.onclick = () => {
    const group = button.closest('.client-group')
    const expanded = group.classList.toggle('expanded')
    button.querySelector('.toggle-icon').textContent = expanded ? '▲' : '▼'
    button.setAttribute('aria-label', expanded ? 'Recolher pedidos do cliente' : 'Expandir pedidos do cliente')
    button.setAttribute('title', expanded ? 'Recolher' : 'Expandir')
  })
}
...
async function administrationLegacy() { if (currentUser?.role !== 'owner') return overview(); layout('Administracao', `<div class="page-actions"><p class="muted">Crie e gerencie os acessos ao sistema.</p></div><div class="split-layout"><div class="form-card"><div class="card-title"><div><h3>Novo usuario</h3><p>Cada usuario recebe um dashboard separado.</p></div><span class="step">01</span></div><form id="user-form"><label>Nome completo<input name="name" required placeholder="Ex: Marina Costa"></label><label>Usuario<input name="user" required placeholder="Ex: marina"></label><label>Email<input name="email" type="email" required placeholder="marina@empresa.com"></label><label>Nome da empresa<input name="companyName" placeholder="Empresa do usuario"></label><label>Senha<div class="generated-password"><input name="password" type="text" minlength="8" required><button type="button" class="icon-button" id="generate-password" title="Gerar senha" aria-label="Gerar senha">↻</button></div></label><button type="submit" class="primary full">Criar usuario</button></form></div><div class="list-card"><div class="card-title"><div><h3>Usuarios cadastrados</h3><p>Contas ativas podem acessar o sistema.</p></div></div><div id="users-list"><div class="empty"><strong>Carregando usuarios...</strong></div></div></div></div>`, 'Controle de acesso')
  ...
  const generatePassword = () => { const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789'; password.value = Array.from({ length: 8 }, () => chars[crypto.getRandomValues(new Uint32Array(1))[0] % chars.length]).join('') }
  document.querySelector('#generate-password').onclick = generatePassword
  generatePassword()
  form.onsubmit = async (event) => { event.preventDefault(); const fields = new FormData(form); const button = form.querySelector('button[type=submit]'); button.disabled = true; try { const response = await fetch(apiUrl('/api/admin/users'), { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(Object.fromEntries(fields)) }); const contentType = response.headers.get('content-type') || ''; const result = contentType.includes('application/json') ? await response.json() : {}; if (!response.ok) throw new Error(result.error || `A API retornou o erro ${response.status}.`); flash('Usuario criado com sucesso'); form.reset(); generatePassword(); await loadAdminUsers() } catch (error) { flash(error.message) } finally { button.disabled = false } }
  await loadAdminUsers()
}
async function loadAdminUsersLegacy() { const container = document.querySelector('#users-list'); if (!container) return; try { const response = await fetch(apiUrl('/api/admin/users'), { headers: { Authorization: `Bearer ${token}` } }); const users = await response.json(); if (!response.ok) throw new Error(users.error); container.innerHTML = users.map((user) => `<div class="user-line"><span class="avatar soft">${initials(user.name)}</span><div class="client-info"><strong>${user.name}</strong><small>@${user.user} · ${user.email}</small></div><span class="status ${user.active ? 'active' : 'inactive'}"><i></i>${user.active ? 'Ativo' : 'Inativo'}</span><button class="danger-link" data-toggle-user="${user.id}" data-user-active="${user.active}">${user.active ? 'Inativar' : 'Ativar'}</button></div>`).join('') || empty('Nenhum usuario cadastrado'); container.querySelectorAll('[data-toggle-user]').forEach((button) => button.onclick = async () => { const response = await fetch(apiUrl(`/api/admin/users/${button.dataset.toggleUser}/status`), { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ active: button.dataset.userActive !== 'true' }) }); if (!response.ok) { const result = await response.json(); return flash(result.error || 'Nao foi possivel alterar o usuario.') } await loadAdminUsersLegacy() }) } catch (error) { container.innerHTML = `<div class="empty"><strong>${error.message || 'Nao foi possivel carregar os usuarios.'}</strong></div>` } }
async function administration() { if (currentUser?.role !== 'owner') return overview(); layout('Administracao', `<div class="page-actions"><p class="muted">Crie empresas e gerencie os acessos ao sistema.</p></div><div class="admin-cards"><div class="form-card"><div class="card-title"><div><h3>Criar empresa</h3><p>Prepare um novo dashboard para uma conta.</p></div><span class="step">01</span></div><form id="company-form"><label>Nome da empresa<input name="name" required placeholder="Ex: Doceria"></label><button type="submit" class="primary full">Criar empresa</button></form></div><div class="form-card"><div class="card-title"><div><h3>Criar usuario</h3><p>Vincule o acesso a uma empresa existente.</p></div><span class="step">02</span></div><form id="user-form"><label>Nome completo<input name="name" required placeholder="Nome Completo do usuário"></label><label>Usuario<input name="user" required placeholder="Usuário"></label><label>Email<input name="email" type="email" required placeholder="usuario@empresa.com"></label><label>Permissao<select name="role"><option value="user">Usuario</option><option value="owner">Administrador</option></select></label><label>Empresa existente<select name="companyId" id="company-id" required><option value="">Carregando empresas...</option></select></label><label>Senha<div class="generated-password"><input name="password" type="text" minlength="8" required><button type="button" class="icon-button" id="generate-password" title="Gerar senha" aria-label="Gerar senha">↻</button></div></label><button type="submit" class="primary full">Criar usuario</button></form></div></div><div class="list-card admin-user-list"><div class="card-title"><div><h3>Usuarios cadastrados</h3><p>A senha nunca aparece nesta lista.</p></div></div><div id="users-list"><div class="empty"><strong>Carregando usuarios...</strong></div></div></div>`, 'Controle de acesso')
  document.querySelector('.admin-cards').insertAdjacentHTML('afterend', '<div class="list-card admin-company-list"><div class="card-title"><div><h3>Empresas cadastradas</h3><p>A empresa do administrador nao aparece para vinculo.</p></div></div><div id="companies-list"><div class="empty"><strong>Carregando empresas...</strong></div></div></div>')
  const companyForm = document.querySelector('#company-form'); companyForm.onsubmit = async (event) => { event.preventDefault(); const button = companyForm.querySelector('button[type=submit]'); button.disabled = true; try { const response = await fetch(apiUrl('/api/admin/companies'), { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(Object.fromEntries(new FormData(companyForm))) }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.error || 'Nao foi possivel criar a empresa.'); flash('Empresa criada com sucesso'); companyForm.reset(); await loadAdminCompanies(); await loadAdminUsers() } catch (error) { flash(error.message) } finally { button.disabled = false } }
  const form = document.querySelector('#user-form'); const password = form.querySelector('[name=password]'); const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789'; const generatePassword = () => { password.value = Array.from({ length: 8 }, () => chars[crypto.getRandomValues(new Uint32Array(1))[0] % chars.length]).join('') }; document.querySelector('#generate-password').onclick = generatePassword; generatePassword(); await loadAdminCompanies()
  form.onsubmit = async (event) => { event.preventDefault(); const button = form.querySelector('button[type=submit]'); button.disabled = true; try { const response = await fetch(apiUrl('/api/admin/users'), { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(Object.fromEntries(new FormData(form))) }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.error || `A API retornou o erro ${response.status}.`); flash('Usuario criado com sucesso'); form.reset(); generatePassword(); await loadAdminUsers() } catch (error) { flash(error.message) } finally { button.disabled = false } }
  await loadAdminUsers()
}
async function loadAdminCompanies() { const select = document.querySelector('#company-id'); const list = document.querySelector('#companies-list'); if (!select && !list) return; try { const response = await fetch(apiUrl('/api/admin/companies'), { headers: { Authorization: `Bearer ${token}` } }); const contentType = response.headers.get('content-type') || ''; const companies = contentType.includes('application/json') ? await response.json() : []; if (!response.ok) throw new Error(companies.error || `A API retornou o erro ${response.status}.`); if (select) select.innerHTML = companies.map((company) => `<option value="${company.id}">${company.name}</option>`).join('') || '<option value="">Nenhuma empresa criada</option>'; if (list) list.innerHTML = companies.map((company) => `<div class="company-line"><span class="product-dot">⌂</span><div class="client-info"><strong>${company.name}</strong><small>Dashboard disponivel para vinculo</small></div></div>`).join('') || '<div class="empty"><strong>Nenhuma empresa criada</strong></div>' } catch (error) { if (select) select.innerHTML = '<option value="">Nao foi possivel carregar</option>'; if (list) list.innerHTML = `<div class="empty"><strong>${error.message || 'Nao foi possivel carregar as empresas.'}</strong></div>`; flash(error.message || 'Nao foi possivel carregar as empresas.') } }
async function loadAdminUsers() { const container = document.querySelector('#users-list'); if (!container) return; try { const response = await fetch(apiUrl('/api/admin/users'), { headers: { Authorization: `Bearer ${token}` } }); const users = await response.json(); if (!response.ok) throw new Error(users.error || 'Nao foi possivel carregar os usuarios.'); container.innerHTML = users.map((user) => `<div class="user-line"><span class="avatar soft">${initials(user.name)}</span><div class="client-info"><strong>${user.name}</strong><small>@${user.user} · ${user.company_name} · ${user.role === 'owner' ? 'Administrador' : 'Usuario'}</small></div><span class="status ${user.active ? 'active' : 'inactive'}"><i></i>${user.active ? 'Ativo' : 'Inativo'}</span><button class="mini-button" data-edit-user="${user.id}">Editar</button><button class="danger-link" data-reset-user="${user.id}">Resetar senha</button><button class="danger-link" data-toggle-user="${user.id}" data-user-active="${user.active}">${user.active ? 'Inativar' : 'Ativar'}</button><button class="danger-icon" data-delete-user="${user.id}" aria-label="Excluir usuario" title="Excluir usuario">×</button></div>`).join('') || empty('Nenhum usuario cadastrado'); container.querySelectorAll('[data-toggle-user]').forEach((button) => button.onclick = async () => { const response = await fetch(apiUrl(`/api/admin/users/${button.dataset.toggleUser}/status`), { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ active: button.dataset.userActive !== 'true' }) }); const result = await response.json().catch(() => ({})); if (!response.ok) return flash(result.error || 'Nao foi possivel alterar o usuario.'); await loadAdminUsers() }); container.querySelectorAll('[data-edit-user]').forEach((button) => button.onclick = () => openAdminUserModal(users.find((user) => user.id === button.dataset.editUser))); container.querySelectorAll('[data-reset-user]').forEach((button) => button.onclick = () => resetAdminPassword(button.dataset.resetUser)) } catch (error) { container.innerHTML = `<div class="empty"><strong>${error.message}</strong></div>` } }
function openAdminUserModal(user) { const modal = document.createElement('div'); modal.className = 'confirm-modal show'; modal.innerHTML = `<div class="confirm-card" role="dialog" aria-modal="true"><span class="confirm-icon">✎</span><h2>Editar usuario</h2><form id="admin-edit-form" class="password-form"><label>Nome<input name="name" value="${user.name}" required></label><label>Usuario<input name="user" value="${user.user}" required></label><label>Email<input name="email" type="email" value="${user.email}" required></label><label>Permissao<select name="role"><option value="user" ${user.role === 'user' ? 'selected' : ''}>Usuario</option><option value="owner" ${user.role === 'owner' ? 'selected' : ''}>Administrador</option></select></label><div class="confirm-actions"><button type="button" class="ghost" data-close-admin-modal>Cancelar</button><button type="submit" class="primary">Salvar</button></div></form></div>`; document.body.appendChild(modal); const close = () => modal.remove(); modal.querySelector('[data-close-admin-modal]').onclick = close; modal.onclick = (event) => { if (event.target === modal) close() }; modal.querySelector('form').onsubmit = async (event) => { event.preventDefault(); const response = await fetch(apiUrl(`/api/admin/users/${user.id}`), { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); const result = await response.json().catch(() => ({})); if (!response.ok) return flash(result.error || 'Nao foi possivel editar o usuario.'); close(); flash('Usuario atualizado'); await loadAdminUsers() } }
async function resetAdminPassword(userId) { const response = await fetch(apiUrl(`/api/admin/users/${userId}/reset-password`), { method: 'POST', headers: { Authorization: `Bearer ${token}` } }); const result = await response.json().catch(() => ({})); if (!response.ok) return flash(result.error || 'Nao foi possivel resetar a senha.'); const modal = document.createElement('div'); modal.className = 'confirm-modal show'; modal.innerHTML = `<div class="confirm-card" role="dialog" aria-modal="true"><span class="confirm-icon">*</span><h2>Senha temporaria gerada</h2><p>Copie esta senha e entregue ao usuario. Ela nao sera exibida novamente.</p><div class="generated-result"><code>${result.temporaryPassword}</code><button type="button" class="icon-button" data-copy-password title="Copiar senha" aria-label="Copiar senha">⧉</button></div><div class="confirm-actions"><button type="button" class="primary" data-close-reset>Concluir</button></div></div>`; document.body.appendChild(modal); modal.querySelector('[data-copy-password]').onclick = async () => { await navigator.clipboard.writeText(result.temporaryPassword); flash('Senha copiada') }; modal.querySelector('[data-close-reset]').onclick = () => modal.remove() }
function settings() { layout('Configuracoes', `<div class="settings-wrap"><div class="form-card"><div class="card-title"><div><h3>Dados da empresa</h3><p>Essas informacoes aparecem no painel.</p></div><span class="step">01</span></div><form id="settings-form"><label>Nome da empresa<input name="company" value="${state.company}" required></label><button class="primary full">Salvar alteracoes</button></form><div class="settings-actions"><button type="button" class="outline-button full" data-change-password>Alterar senha</button><button type="button" class="logout-button full" data-logout>Sair da conta</button></div></div><div class="settings-note"><span class="brand-mark">P</span><h3>Pedi<span>flow</span></h3><p>Gestao simples para quem faz acontecer.</p><small>Dados salvos neste dispositivo.</small></div></div>`, 'Painel de operacao'); bind(); document.querySelector('#settings-form').onsubmit = (e) => { e.preventDefault(); state.company = new FormData(e.target).get('company'); save(); flash('Configuracoes salvas'); settings(); enhanceSettings() }; document.querySelector('[data-change-password]').onclick = openPasswordModal; document.querySelector('[data-logout]').onclick = logout }
function logout() { token = null; localStorage.removeItem(authKey); state = null; showLogin('Sessao encerrada.') }
function closePasswordModal() { document.querySelector('#password-modal')?.remove() }
function addPasswordToggle(input) { const wrapper = document.createElement('div'); wrapper.className = 'password-field'; input.parentNode.insertBefore(wrapper, input); wrapper.appendChild(input); const button = document.createElement('button'); button.type = 'button'; button.className = 'password-toggle'; button.dataset.passwordToggle = ''; button.setAttribute('aria-label', 'Mostrar senha'); button.title = 'Mostrar senha'; button.textContent = '◉'; wrapper.appendChild(button) }
function openPasswordModal() { const modal = document.createElement('div'); modal.id = 'password-modal'; modal.className = 'confirm-modal show'; modal.innerHTML = `<div class="confirm-card" role="dialog" aria-modal="true" aria-labelledby="password-title"><span class="confirm-icon">&#9679;</span><h2 id="password-title">Alterar senha</h2><p>Defina uma nova senha para proteger o acesso ao seu dashboard.</p><form id="password-form" class="password-form"><label>Senha atual<input name="currentPassword" type="password" autocomplete="current-password" required></label><label>Nova senha<input name="newPassword" type="password" autocomplete="new-password" minlength="8" required><small>Use pelo menos 8 caracteres.</small></label><p class="login-error" id="password-error"></p><div class="confirm-actions"><button type="button" class="ghost" data-password-cancel>Cancelar</button><button type="submit" class="primary">Salvar senha</button></div></form></div>`; document.body.appendChild(modal); modal.querySelector('[data-password-cancel]').onclick = closePasswordModal; modal.onclick = (event) => { if (event.target === modal) closePasswordModal() }; modal.querySelector('input').focus(); modal.querySelector('#password-form').onsubmit = async (event) => { event.preventDefault(); const form = event.target; const button = form.querySelector('button[type="submit"]'); const errorElement = form.querySelector('#password-error'); const fields = new FormData(form); button.disabled = true; errorElement.textContent = ''; try { const response = await fetch(apiUrl('/api/auth/password'), { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ currentPassword: fields.get('currentPassword'), newPassword: fields.get('newPassword') }) }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.error || 'Nao foi possivel alterar a senha.'); closePasswordModal(); flash('Senha alterada com sucesso') } catch (error) { errorElement.textContent = error.message; button.disabled = false } } }
...
document.addEventListener('click', (e) => { const button = e.target.closest('[data-delete-client],[data-delete-product],[data-delete-order],[data-delete-list],[data-delete-user]'); if (!button) return; e.preventDefault(); e.stopImmediatePropagation(); requestDelete(button) }, true)
...
function addShoppingPreviewButtons() { document.querySelectorAll('.saved-list').forEach((item) => { if (item.querySelector('[data-view-list]')) return; const exportButton = item.querySelector('[data-export-list]'); const editButton = item.querySelector('[data-edit-list]'); if (!exportButton) return; exportButton.insertAdjacentHTML('beforebegin', `<button class="mini-button" data-view-list="${exportButton.dataset.exportList}">Visualizar</button>`); if (editButton) exportButton.before(editButton) }) }
function closeShoppingPreview() { document.querySelector('.shopping-preview-modal')?.remove() }
function showShoppingPreview(list) { const modal = document.createElement('div'); modal.className = 'shopping-preview-modal'; modal.innerHTML = `<div class="shopping-preview-card" role="dialog" aria-modal="true" aria-labelledby="shopping-preview-title"><div class="shopping-preview-head"><div><span class="kicker">LISTA DE COMPRAS</span><h2 id="shopping-preview-title">Lista de compras</h2><p>${date(list.createdAt)} · ${list.items.length} itens</p></div><button class="icon-button shopping-preview-close" aria-label="Fechar visualizacao" title="Fechar">×</button></div><div class="shopping-preview-items">${list.items.map((item) => `<div><strong>${item.quantity}</strong><span>${item.name}</span></div>`).join('')}</div></div>`; document.body.appendChild(modal); modal.addEventListener('click', (event) => { if (event.target === modal || event.target.closest('.shopping-preview-close')) closeShoppingPreview() }) }
document.addEventListener('click', (e) => { const button = e.target.closest('[data-view-list]'); if (!button) return; const list = state.shopping.find((item) => item.id === button.dataset.viewList); if (list) showShoppingPreview(list) })
...
function showLogin(message = '') {
  ...
  document.querySelector('#app').innerHTML = `<main class="login-shell"><section class="login-panel"><div class="login-brand"><span class="brand-mark">P</span><div><strong>Pedi<span>flow</span></strong><small>gestao de pedidos</small></div></div><div class="login-copy"><span class="login-kicker">AREA RESTRITA</span><h1>Seu fluxo, no lugar certo.</h1><p>Entre para acessar o dashboard da sua conta.</p></div><form id="login-form" class="login-form"><label>Usuario<input name="user" type="text" autocomplete="username" placeholder="Seu usuario" required></label><label>Senha<div class="password-field"><input name="password" type="password" autocomplete="current-password" placeholder="Sua senha" required><button type="button" class="password-toggle" data-password-toggle aria-label="Mostrar senha" title="Mostrar senha">◉</button></div></label><p class="login-error" id="login-error">${message}</p><button class="login-submit">Entrar no dashboard <span>→</span></button></form></section><aside class="login-aside"><div class="login-orbit orbit-one"></div><div class="login-orbit orbit-two"></div><div class="login-aside-content"><span>PD</span><strong>Organizacao que<br>acompanha seu ritmo.</strong><small>Pedidos, clientes e produtos em um unico lugar.</small></div></aside></main>`
  ...
  form.onsubmit = async (event) => {
    ...
    button.innerHTML = 'Validando acesso <span>...</span>'
    error.textContent = ''
    try {
      const fields = new FormData(form)
      const response = await fetch(apiUrl('/api/auth/login'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user: fields.get('user'), password: fields.get('password') }) })
      ...
      button.innerHTML = 'Entrar no dashboard <span>→</span>'
    }
  }
}
...
async function bootstrap() {
  try {
    ...
    const response = await fetch(apiUrl('/api/state'), { headers: { Authorization: `Bearer ${token}` } })
    if (response.status === 401) { token = null; localStorage.removeItem(authKey); return showLogin('Entre novamente para continuar.') }
    if (!response.ok) {
      const details = await response.json().catch(() => ({}))
      throw new Error(details.error || `A API retornou o erro ${response.status}.`)
    }
